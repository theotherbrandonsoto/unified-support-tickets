-- models/staging/stg_silo_d_program_d.sql
-- Normalize Silo D Program D data: standardize column names, convert to UTC, map status values

with source as (
    select * from {{ source('raw', 'raw_silo_D_program_d') }}
),
...
with_status_mapping as (
    ...
    left join {{ ref('status_mapping') }} sm
        ...
        and sm.source_program = 'raw_silo_D_program_d'
)
