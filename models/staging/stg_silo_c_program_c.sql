-- models/staging/stg_silo_c_program_c.sql
-- Normalize Silo C Program C data: standardize column names, convert to UTC, map status values

with source as (
    select * from {{ source('raw', 'raw_silo_C_program_c') }}
),
...
with_status_mapping as (
    ...
    left join {{ ref('status_mapping') }} sm
        ...
        and sm.source_program = 'raw_silo_C_program_c'
)
