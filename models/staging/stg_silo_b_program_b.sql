-- models/staging/stg_silo_b_program_b.sql
-- Normalize Silo B Program B data: standardize column names, convert to UTC, map status values

with source as (
    select * from {{ source('raw', 'raw_silo_B_program_b') }}
),
...
with_status_mapping as (
    ...
    left join {{ ref('status_mapping') }} sm
        ...
        and sm.source_program = 'raw_silo_B_program_b'
)
